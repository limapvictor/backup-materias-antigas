...
public class Board 
{
    ...
    public String toString()
    {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++)
                tab += String.format("%4d", tabuleiro[i][j]);
            tab += "\n";
        }
        return (tab);
    }
    ...
    public Iterable<Board> neighbors()
    {
        ...
        Queue<Board> vizinhos = new Queue<Board>();
        int lin = achaQuadrado()[0], col = achaQuadrado()[1];
        if (lin != 0)
            vizinhos.enqueue(new Board(moveQuadrado(lin, col, lin - 1, col)));
            // vizinhos.enqueue(new Board(moveQuadrado(lin, col, lin - 1, col)));
        if (lin != N - 1)
            vizinhos.enqueue(new Board(moveQuadrado(lin, col, lin + 1, col)));
            // vizinhos.enqueue(new Board(moveQuadrado(lin, col, lin + 1, col)));
        if (col != 0)
            vizinhos.enqueue(new Board(moveQuadrado(lin, col, lin, col - 1)));
            // vizinhos.enqueue(new Board(moveQuadrado(lin, col, lin, col - 1)));
        if (col != N - 1)
            vizinhos.enqueue(new Board(moveQuadrado(lin, col, lin, col + 1)));
            // vizinhos.enqueue(new Board(moveQuadrado(lin, col, lin, col + 1)));
    ...
    public static void main(String args[])
    {
        ...
        StdOut.println("Posição [0,0] -> " + b.tileAt(0, 0));
