...
class Personagem:
    ...
    def perceber(self,percepcao,MundoW):    
        """ Método encarregado de montar a percepção que será entregue ao personagem para que ele possa fazer o planejamento correto.
        """
        pos = self.posicao
        # pego todos os vizinhos da posição atual do personagem e verifico se há algum perigo nas redondezas.
        ...
        # verifico se na ultima tentativa de movimentação o personagem bateu em um muro.
        if(self.impacto):
            percepcao.append("I")
            self.impacto = False
        if(MundoW.urro and "U" not in percepcao):
            percepcao.append("U")
        # por fim coloco o nome do personagem que será usado para fim do compartilhamento de informações.
        percepcao.append(self.nome)
        return percepcao

    def planejar(self,percep):
        """ Método que chama o planejamento dos personagens no módulo, enviando a percepção naquele sala atual.
        """
    ...
    def morrer(self,MundoW):
        """ Método que ajusta alguns valores e atualiza a lista de personagens após a morte do personagem.
        """
        self.estaviva = False
        self.posicao = []
        self.modulo = None
    ...
    def compartilhe(self,infoComp):
        ...
        for i in range(self.N):
            for j in range(self.N):
                ...
                if(ori1 == 0):
                    # apenas adiciono informações novas ao mundo compartilhado do usuário, para não poluir demais as informações recebidas.
            ...
            for j in range(self.N):
                if(ori1 == 0):
                    for info in self.modulo.mundo[ori0*(i-i0)][j-j0]:
                        # como o personagem ja visitou a sala e continua vivo, aquela sala está livre de perigos e 
                        # isso pode ser informado aos outros usúarios.
                        if(info == "V"):
                            matriz[i][j] += "L"
                        # só adiciono informações novas a matriz que será compartilhada depois.
...
class ListaDePersonagens:
    """ Classe que lida com a lista de personagens como um todo, com uma visão mais geral,
        em contrapartida com a classe personagem que lida com cada personagem de uma forma bem 
        mais individual.
    """
    def __init__(self,N,nPersonagens,mundo):
        """ Construção da lista de personagens.
        """
        self.N = N
    ...
    def processaPercepcoes(self,MundoW):
        """ Método que monta uma matriz fiel ao mundo de wumpus de acordo com a percepção de todos os personagens.
        """
        # criação da matriz perrcepções, fiel ao mundo de wumpus.
        self.percepcoes = []
        for i in range(self.N):
            linha = []
            for j in range(self.N):
        ...
        # inclusão da percepção dos personagens referentes a posição deles na matri percepções.
        for personagem in self.personagens:
    ...
    def processaPlanejamentos(self):
        """ Nesse método cada personagem recebe a percepção referente a sua sala para poder invocar
            o método planejar do módulo.
        """
        for personagem in self.personagens:
            personagem.planejar(self.percepcoes[personagem.posicao[0]][personagem.posicao[1]])
    
    def processaCompartilhamentos(self):
        """ Método que processa os compartilhamentos antes das ações devido a presença de mais de uma personagem na execução.
        """
        # primeiro, utilizando-se do modo em que a matriz percepções foi montada, no qual ela indica quais personagens
        # estão naquela sala, eu descubro em que salas ocorrerá o compartilhamento de informações.
        ...
        # depois vou compartilhando sala por sala
        for sala in salas:
            # primeiro crio a matriz infoComp, que será a responsável por abrigar todas informações que cada usuário esta compartilhando,
            # e depois repassára a cada usuário o resultado final dessa união de informações.
            infoComp = []
            for i in range(self.N):
                lin = []
                for j in range(self.N):
            ...
            for k in range(2):
                for percepcao in sala:
                    #verifico as percepções da sala, se ela não for uma das padrões então ela é o nome de um personagem presente naquela sala.
                    if(percepcao not in ["F","B","I","U"]):
                        if(k == 0):
                            # na primeira passada do loop, eu adiciono o conhecimento de mundo do personagem na matriz infoComp 
                            infoComp = self.dicPersonagens[percepcao].adicionarInfoComp(infoComp)
                        else:
                            # na segunda passada do loop o personagem recebe a reunião das informações compartilhadas pelos usuários.
                            self.dicPersonagens[percepcao].compartilhe(infoComp)

    def processaTiros(self,MundoW):
        """ A ação de atirar tem a preferência pois a morte de um Wumpus pode salvar um personagem que caminhava a morte certa.
        """
        for i in range(self.nPersonagens):
            if(self.acoes[i] == "T"):
    ...
    def processaAcoes(self,MundoW):
        """ Método que invoca o método agir do módulo do personagem e então faz as operações necessárias para realizar
            a ação no mundo real de Wumpus.
        """
        self.acoes = []
        for personagem in self.personagens:
...
class Interface:
    """ Classe que implementa a interface do mundo através da biblioteca pygame,
        através de recursos visuais e sonoros.
    """
    def __init__(self,N,mundo,personagens):
        """ Construtor que inicia a interface do pygame.
        """
        self.N = N 
    ...
    def atualizaTela(self, mundo, personagens):
        """ Método que a cada rodada atualiza a interface para ficar condizente 
            com o estado do mundo e dos personagens, mostrando as intenções e
            ações dos personagens.
        """
        # atualização das salas representadas na tela, que pode mudar de acordo com mudanças
        # nos atributos i0,j0,S ou até mesmo após a morte de um Wumpus.
        for i in range(self.i0,self.S):
            for j in range(self.j0,self.S):
                tam = self.M // self.S #o tamanho que cada sala terá
                tipo = mundo[(self.i0+i)%self.N][(self.j0+j)%self.N] # verificação do tipo da sala 
                sala = pygame.transform.scale(self.imagens[tipo], (tam, tam)) # redimensionamento da imagem
                self.tela.blit(sala, [tam*j,tam*i]) # atualização da interface
        orientacoes = [[0,1],[-1,0],[0,-1],[1,0]] # esse vetor está organizado de modo a facilitar a rotação da seta que indica a orientação da personagem.
        salas = []
        for personagem in personagens:
            # apenas atualizo as posições que estarão presentes na tela, para economizar um pouco de processamento.
            if(self.i0 <= personagem.posicao[0] < self.S and self.j0 <= personagem.posicao[1] < self.S): 
                tam = self.M // self.S
                # se aquela sala ja está no vetor salas quer dizer que um personagem ja se encontra nela,
                # logo naquela sala ocorrerá um compartilhamento de informações.
                ...
                # se a sala não está no vetor salas, o personagem é o unico na sala até aquele momento.
                else:
                    # impressão da representação de personagem na sala em que ele se encontra
                    ...
                    # impressão da orientação do personagem 
    ...
    def processaComando(self, MundoW):
        """ Método que processa os comandos do usuário relativos a interface e ao funcionamento do mundo.
        """
    ...
    def reproduzSom(self, som):
        """ Método que reproduz sons de acordo com ações chaves ocorridas no mundo.
        """
