...
int isEmptyControlPoint(Grid *g, Position p){
    /*Código base dos exemplos fornecidos*/
    return (g->map[p.x][p.y].isControlPoint && (g->map[p.x][p.y].type == NONE));
    /*Retorna se o tile analisado é um ControlPoint e ele está vazio*/
}
...
Action defender(Grid *g, Position p, Robot *r){
    int enemy = searchNearestEnemy(g, p, r);
    if (enemy == 4)
